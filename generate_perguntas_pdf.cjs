...
const htmlPath = path.join(__dirname, 'tcc_perguntas_banca.html');
const pdfPath = path.join(__dirname, 'tcc_perguntas_banca.pdf');
...
if (!chromeExe) {
  ...
  console.log('Por favor, abra o arquivo "tcc_perguntas_banca.html" no seu navegador e imprima como PDF manualmente.');
  process.exit(1);
}

console.log(`Google Chrome encontrado em: ${chromeExe}`);
console.log('Gerando PDF do Guia da Banca. Por favor, aguarde...');
...
exec(command, (error, stdout, stderr) => {
  ...
  if (fs.existsSync(pdfPath)) {
    console.log('--------------------------------------------------');
    console.log('PDF do Guia da Banca Gerado com Sucesso!');
